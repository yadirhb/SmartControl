		...
		protected void onPostExecute(Object result) {
			if (resultado1.get(0).get("status").equals("1")) {
				...
				if (resultado1.size() > 1) {
					editor.putString("fecha_hora",
							resultado1.get(1).get("fecha_hora").toString());
					editor.commit();
					
					for (int i = 1; i < resultado1.size(); i++) {
						ContentValues valor = new ContentValues();
						valor.put("nombre_user", resultado1.get(i)
								.get("nombre").toString());
						valor.put(
								"tipo_evento",
								Integer.parseInt(resultado1.get(i)
										.get("tipo_evento").toString()));
						valor.put("fecha_evento", resultado1.get(i)
								.get("fecha").toString());
						valor.put("hora_evento", resultado1.get(i).get("hora")
								.toString());
						valor.put("puerta", resultado1.get(i).get("puerta")
								.toString());
						valor.put("url_audio",
								resultado1.get(i).get("url_audio").toString());
						valor.put("url_foto", resultado1.get(i).get("url_img")
								.toString());
